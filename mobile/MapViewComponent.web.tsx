...
<body>
  ...
  <script>
    ...
    // Nominatim geocoding (OSM 무료)
    function searchPlace(query) {
      if (!query || query.length < 2) {
        document.getElementById('search-results').className = 'search-results';
        return;
      }
      fetch('https://nominatim.openstreetmap.org/search?format=json&q=' + encodeURIComponent(query) + '&countrycodes=kr&limit=5&accept-language=ko', {
      ...
      .then(function(results) {
        ...
        if (!results || results.length === 0) {
          container.innerHTML = '<div class="search-result-item"><div class="search-result-name">검색 결과 없음</div></div>';
          container.className = 'search-results active';
          return;
        }
        container.innerHTML = results.map(function(r) {
          ...
          return '<div class="search-result-item" data-lat="' + r.lat + '" data-lng="' + r.lon + '" data-name="' + name + '">'
        ...
        container.querySelectorAll('.search-result-item').forEach(function(item) {
          item.addEventListener('click', function() {
            var rlat = parseFloat(this.dataset.lat);
            var rlng = parseFloat(this.dataset.lng);
            var rname = this.dataset.name;
            ...
            searchMarker = L.marker([rlat, rlng], { icon: pinIcon }).addTo(map)
              .bindPopup('<b>' + rname + '</b>').openPopup();

            // 검색 결과 닫기
            container.className = 'search-results';
            document.getElementById('map-search').value = rname;

            // 해당 위치 기준으로 산책로 + POI 다시 로드
            loadTrails(rlat, rlng);
            loadPOIs(rlat, rlng);
          });
        });
      })
      .catch(function(e) { console.log('Search error:', e); });
    }
